...
# they point to the env vars set by Kubernetes that contain the actual
# target address and port. Override the default with them.
if [ -n "${SERVICE_HOST_ENV_NAME2+1}" ]; then
  TARGET_SERVICE2=${!SERVICE_HOST_ENV_NAME2}
fi
if [ -n "${SERVICE_PORT_ENV_NAME2+1}" ]; then
  TARGET_SERVICE2="$TARGET_SERVICE2:${!SERVICE_PORT_ENV_NAME2}"
fi

# Tell nginx the address and port of the service to proxy to
sed -i "s/{{TARGET_SERVICE2}}/${TARGET_SERVICE2}/g;" /etc/nginx/conf.d/proxy.conf
